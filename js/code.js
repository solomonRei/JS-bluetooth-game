document.getElementsByClassName('playSend')[0].style.display = "none";
...
function resetGame(){
  ...
  for (var i = 0; i < mapProcessor.getEnemyPositions().length; i++) {
    enemyPosition = mapProcessor.getEnemyPositions()[i];
  ...
  for (var i = 0; i < mapProcessor.getWallPositions().length; i++) {
    wallPosition = mapProcessor.getWallPositions()[i];
    wall = new Wall(wallPosition.x, wallPosition.y);
    entities.push(wall);
    walls.push(wall);
  }
